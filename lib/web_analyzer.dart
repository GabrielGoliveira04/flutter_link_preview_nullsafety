...
/// Web analyzer
class WebAnalyzer {
  static final Map<String, WebInfo> _map = {};
  ...
  /// Get web information
  /// return [InfoBase]
  static WebInfo? getInfoFromCache(String url) {
    final WebInfo? info = _map[url];
    if (info != null) {
  ...
  /// Get web information
  /// return [InfoBase]
  static Future<WebInfo?> getInfo(String url,
      {Duration cache = const Duration(hours: 24),
      bool multimedia = true,
      bool useMultithread = false}) async {
    // final start = DateTime.now();

    WebInfo? info = getInfoFromCache(url);
  ...
  static Future<WebInfo?> _getInfo(String url, bool multimedia) async {
  ...
  static Future<WebInfo> _getInfoByIsolate(String url, bool multimedia) async {
  ...
  static Future<WebInfo?> _getWebInfo(
    ...
    if (response.statusCode == HttpStatus.ok) {
      ...
      // get image or video
      if (multimedia) {
        final WebInfo? gif = _analyzeGif(document, uri);
        if (gif != null) return gif;

        final WebInfo? video = _analyzeVideo(document, uri);
  ...
  static WebInfo? _analyzeGif(Document document, Uri uri) {
  ...
  static WebInfo? _analyzeVideo(Document document, Uri uri) {
